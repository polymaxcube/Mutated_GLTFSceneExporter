...
#region Profiler Markers
		...
		private NodeId ExportNode(Transform nodeTransform)
		{
			...
			// 오브젝트에서 GltfMetadata 컴포넌트를 가져와 JsonUtility로 JSON으로 변환한 뒤, node.Extras에 넣어 GLB 파일에 메타데이터를 포함시킵니다.
			var metadataComp = nodeTransform.GetComponent("GltfMetadata");		
			if (metadataComp != null)
			{
				string json = JsonUtility.ToJson(metadataComp);
				node.Extras = Newtonsoft.Json.Linq.JToken.Parse(json);
			}
